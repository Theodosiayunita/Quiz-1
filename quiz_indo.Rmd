...
data <- read.csv("E:/XION B/Week 1/Kamis-Jumat/v1_quiz_1_p4ds_ps-main/v1_quiz_1_p4ds_ps-main/datainput/retail.csv")
...
```{r}
# code Here
rata_profit <- mean(data$Profit)
rata_profit
```
#mengecek soal
```{r}
nomor1 <- data[data$Profit > rata_profit & data$Quantity < 5,]
nomor1
```
...
```{r}
# code here
FrekuensiFC <- table(data$Segment[data$Ship.Mode == "First Class"])
FrekuensiFC
```
...
```{r}
# code here
Crosstab <- xtabs(Quantity ~ Sub.Category, data = data)
Crosstab
```
#mengurutkan
```{r}
sort_Crosstab <- sort(Crosstab, decreasing=T)
Populer <- names(head(sort_Crosstab,3))
Populer
```
...
```{r}
# code here
Korelasi <- cor(data$Sales, data$Profit)
Korelasi
...
*Referensi Opsi Bahasa Inggris:*
  - [X] Fail to reject the null hypothesis, the promo does not have a statistically significant impact
...
#pengoperasian 
```{r}
z_score <- (sampel-mean)/(sd/sqrt(n))
nilai_kritis <- qnorm(1-(1-cl)/2)
z_score
nilai_kritis
```
